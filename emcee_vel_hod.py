...
def lnprior(theta):
    logMmin, sigma_logM, alpha, logM0, logM1 = theta
    if 1.<logMmin<3. and 0.25<sigma_logM<0.5 and 1.0<alpha<1.3 and 1.<logM0<3. and 2.5<logM1<4.:
        return 0.0
    return -np.inf
...
f = open('param.txt','w')
f.write(str(sampler.chain[np.where(sampler.lnprobability==sampler.lnprobability.max())]))
f.close()
...
plt.yscale('log')
plt.xlabel('step')
plt.ylabel('sigma_logVmax')
plt.savefig('sigmalogVmax_v_step_test.png')
...
plt.xlabel('step')
plt.ylabel('logVmax0')
plt.savefig('logVmax0_v_step_test.png')
...
plt.xlabel('step')
plt.ylabel('logVmax1')
plt.savefig('logVmax1_v_step_test.png')

fig = corner.corner(samples, labels=["$logVmax$", "$sigma_logVmax$", "$alpha$", "$logVmax0", "$logVmax$"],quantiles=(0.16, 0.84), levels=(1-np.exp(-0.5),))
