...
class fit_dict(dict):
    ...
    def _get_data(self):
        if self['data'] == "zehavi":
            ...
            return wp_vals, ng, ng_cov, bin_edges, invcov
...
class hod_fit(fit_dict):
    ...
    def _get_lnlike(self,theta):
        ...
        wp_vals, ng, ng_cov, bin_edges, invcov = self._get_data()
    ...
    def _get_lnlike_lin(self,theta):
        wp_vals, ng, ng_cov, bin_edges, invcov = self._get_data()
        #model_instance.mock.populate()

        ro,g = theta
        rp = (bin_edges[1:]+bin_edges[:-1])/2.
        wp_calc = rp*(rp/ro)**(-g)*gamma(0.5)*gamma((g-1.)/2.)/gamma(g/2.0)
        self.cur_wp = wp_calc

        wp_diff = wp_vals-wp_calc
        #ng_diff = ng-model_instance.mock.number_density

        return -0.5*np.dot(wp_diff, np.dot(invcov, wp_diff))

    def _get_lnprior_lin(self,theta):
        ro, g = theta
        
        if 0.1 < ro < 450. and -1.0 < g < 2.0:
            return 0.0
        return -np.inf

    def _get_lnprob_lin(self,theta):
        lp = self._get_lnprior_lin(theta)
        ll = self._get_lnlike_lin(theta)
        if not np.isfinite(lp):
            return -np.inf,self.cur_wp
...
def main(args):
    ...
    with Pool() as pool:
        ...
        sampler = emcee.EnsembleSampler(nwalkers, ndim, hod._get_lnprob_lin, 
